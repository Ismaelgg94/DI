...
namespace HojaRol
{
    public partial class SelectorPersonaje : Form
    {
        ...
        private void crearPersonaje(object sender, EventArgs e)
        {
            ...
            creador.ShowDialog();
            //Cuando se vuelve al selector se vuelve a mostrar las imagenes ocultadas.
            mostrarAtrezzo();
        ...
        private void visualizarPersonaje(Personaje personaje)
        {
            ...
            pbAvatar.BackgroundImage = Image.FromFile(Path.GetFullPath(Constantes.PATH_RESOURCES+personaje.getRaza()+personaje.getProfesion()+genero+".png"));
        ...
        {
            //Se ocultan para que no se vean de fondo.
            ocultarAtrezzo();
            Form1 form1 = new Form1(album,Constantes.MODO_MODIFICACION);

            form1.ShowDialog();
            //Cuando se vuelve al selector se vuelve a mostrar las imagenes y el btnSalir.
            mostrarAtrezzo();
        ...
        private void importarAlbum(object sender, EventArgs e)
        {
            ...
            //Solo deja que se cargue archivos .txt
            dialog.DefaultExt = "txt";
            ...
            if (resultado == DialogResult.OK)
                if ((Regex.Match(dialog.FileName, ".txt")).Length > 0)
                {
                    ...
                    album.importarAlbum(dialog.FileName);
                    if (album.numPersonajes() > 0)
                    {
                        visualizarPersonaje(album.getPersonajeActual());
                        btnExportar.Visible = true;
        ...
        private void guardarAlbumAlSalir(object sender, FormClosingEventArgs e)
        {
            //Crea la carpeta en el lugar por defecto, sino está creada.
            if (!Directory.Exists(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + Constantes.DEFAULT_ALBUM_DIRECTORY_PATH))
                Directory.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + Constantes.DEFAULT_ALBUM_DIRECTORY_PATH);

            string ruta = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + Constantes.DEFAULT_ALBUM_DIRECTORY_PATH + Constantes.DEFAULT_ALBUM_FILE_NAME;

            //Guarda el album en una ruta por defecto , para guardar el trabajo realizado automaticamente cuando se cierra.
            album.exportarAlbum(ruta);
        }

        private void cargarAlbumAlEntrar(object sender, EventArgs e)
        {
            string defaultFilePath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + Constantes.DEFAULT_ALBUM_DIRECTORY_PATH + Constantes.DEFAULT_ALBUM_FILE_NAME;
            //Si existe el archivo por defecto lo cargará
            if (File.Exists(defaultFilePath))
            {
                //Carga el album guardado anteriormente guardado, para recargar el trabajo realizado anteriormente.
                album.importarAlbum(defaultFilePath);
                if (album.numPersonajes() > 0)
                    visualizarPersonaje(album.getPersonajeActual());
            }
